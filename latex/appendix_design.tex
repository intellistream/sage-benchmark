...
In this appendix, we provide the implementation details of the current SAGE architecture that were omitted from the main text for brevity. This includes the consolidated package hierarchy, the logical-to-physical graph compilation process, the formal execution loop algorithm, and specifications for optional adapter integration and fault tolerance mechanisms.

\subsection{Detailed Package Structure}
\label{subsec:app_package}
SAGE follows a strict layered architecture ($L1 \rightarrow L5$) where higher layers may only depend on lower layers. Table~\ref{tab:package_structure} summarizes the consolidated package hierarchy and the responsibilities of each layer.
...
L1 & \texttt{isage / sage.foundation} & Foundation & Shared types, \texttt{SagePorts}, configuration, logging \\
L2 & \texttt{isage / sage.runtime} & Runtime Services & Environments, schedulers, services, placement contracts \\
L3 & \texttt{isage / sage.stream} & Core Dataflow & \texttt{DataStream}, transformations, runtime packet types \\
L3 & Optional adapters & Algorithmic / retrieval blocks & \texttt{isagellm}, \texttt{isage-rag}, memory/vector integrations \\
L4 & Benchmark / examples adapters & End-to-end integrations & Local helpers, vector backends, benchmark-specific orchestration \\
L5 & Tooling / apps & User workflows & CLI, applications, benchmark runners \\\bottomrule
\end{tabular}%
}
\end{table}
