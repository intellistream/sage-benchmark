...
class ScalabilityExperiment(ControlPlaneExperiment):
    """
    Q4 ScaleFrontier workload.

    Focuses on throughput and latency trends under higher scale.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Q4 – ScaleFrontier: benchmark throughput/latency trends under "
            "higher scale against different backends."
        ),
...
def main() -> None:  # pragma: no cover
    ...
    if args.dry_run:
        print("[dry-run] Q4 ScaleFrontier – configuration validated. Exiting without running.")
        sys.exit(0)

    loader = ConfigLoader()
    config = loader.get_default_config("Q4")
    config.experiment_section = "Q4"
    if args.quick:
